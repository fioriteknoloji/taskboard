      ...
      {total>0 && (
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            {subtasks.map(sub=>(
              <div key={sub.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', borderRadius:7, background:sub.completed?'#f7f7f5':'#fff', border:'1px solid rgba(0,0,0,0.07)' }}>
      ...
      {assignedProfiles.length>0 && (
        <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:12 }}>
          {assignedProfiles.map(p=>(
            <div key={p.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 10px', background:'#f0efec', borderRadius:99, border:'1px solid rgba(0,0,0,0.08)' }}>
              <Avatar initials={p.initials} size={20} />
              <span style={{ fontSize:12, fontWeight:500, color:'#1a1a1a' }}>{p.full_name}</span>
              <button onClick={()=>remove(assignees.find(a=>a.user_id===p.id)?.id)} style={{ background:'none',border:'none',cursor:'pointer',color:'#9b9b9b',fontSize:14,padding:'0 2px',lineHeight:1 }}>×</button>
            </div>
          ))}
        </div>
      )}
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:14 }}>
        ...
        <button onClick={()=>logTime(minutes,note)} disabled={!minutes||parseInt(minutes)<=0}
          style={{ padding:'0 14px', background:minutes&&parseInt(minutes)>0?'#5e6ad2':'#e0dfdc', color:'#fff', border:'none', borderRadius:6, fontSize:13, fontWeight:500, cursor:minutes&&parseInt(minutes)>0?'pointer':'default', whiteSpace:'nowrap' }}>
          Ekle
        </button>
      </div>

      {/* Log list */}
      {logs.length===0 ? <div style={{ textAlign:'center', padding:'20px 0', fontSize:13, color:'#9b9b9b' }}>Henüz zaman kaydı yok</div> : (
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          {logs.map(log=>(
            <div key={log.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', background:'#f7f7f5', borderRadius:7 }}>
              <span style={{ fontSize:13, fontWeight:600, color:'#1a1a1a', minWidth:48 }}>
                {log.minutes>=60?`${Math.floor(log.minutes/60)}s ${log.minutes%60}dk`:`${log.minutes}dk`}
              </span>
              <span style={{ flex:1, fontSize:12, color:'#6b6b6b' }}>{log.note||'—'}</span>
              <span style={{ fontSize:11, color:'#9b9b9b' }}>{new Date(log.logged_at).toLocaleDateString('tr-TR',{day:'numeric',month:'short'})}</span>
              {log.user_id===profile.id && (
                <button onClick={()=>removeLog(log.id)} style={{ background:'none',border:'none',cursor:'pointer',color:'#c4c2bc',fontSize:14,padding:'0 2px' }}>×</button>
              )}
            </div>
          ))}
        </div>
...
function CustomFieldsPanel({ taskId, isAdmin }) {
  ...
  return (
    <div>
          ...
          {fields.map(field=>(
            <div key={field.id}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:5 }}>
                ...
                {isAdmin && <button onClick={()=>deleteField(field.id)} style={{ background:'none',border:'none',cursor:'pointer',color:'#c4c2bc',fontSize:13 }}>×</button>}
              </div>
              {renderInput(field)}
            </div>
          ))}
      ...
      {fields.length===0 && !showAdd && <div style={{ textAlign:'center',padding:'20px 0',color:'#9b9b9b',fontSize:13 }}>Henüz özel alan yok</div>}
      {isAdmin && (showAdd ? (
        <div style={{ background:'#f7f7f5',borderRadius:10,padding:14,border:'1px solid rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10 }}>
            <div><label style={{ fontSize:11,color:'#9b9b9b',display:'block',marginBottom:4 }}>Alan adı</label><Inp value={newField.name} onChange={e=>setNewField(f=>({...f,name:e.target.value}))} placeholder="Müşteri, Story point…" /></div>
            <div><label style={{ fontSize:11,color:'#9b9b9b',display:'block',marginBottom:4 }}>Tip</label><Sel value={newField.field_type} onChange={e=>setNewField(f=>({...f,field_type:e.target.value}))}>{Object.entries(FIELD_TYPE_LABELS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</Sel></div>
          </div>
          {newField.field_type==='select' && <div style={{ marginBottom:10 }}><label style={{ fontSize:11,color:'#9b9b9b',display:'block',marginBottom:4 }}>Seçenekler (virgülle)</label><Inp value={newField.options} onChange={e=>setNewField(f=>({...f,options:e.target.value}))} placeholder="Düşük, Orta, Yüksek" /></div>}
          <div style={{ display:'flex',gap:8 }}>
  ...
  return (
    <div style={{ position:'relative', flex:1 }}>
      <input ref={inputRef} value={value} onChange={handleChange} onKeyDown={onKeyDown}
        ...
        style={{ width:'100%', border:'1px solid rgba(0,0,0,0.12)', borderRadius:6, padding:'7px 10px', fontSize:13, background:'#fff', color:'#1a1a1a', outline:'none' }}
        onFocus={e=>e.target.style.borderColor='#5e6ad2'} onBlur={e=>{e.target.style.borderColor='rgba(0,0,0,0.12)'; setTimeout(()=>setMentionQuery(null),200)}} />
      {suggestions.length>0 && (
        <div style={{ position:'absolute', bottom:'100%', left:0, right:0, background:'#fff', border:'1px solid rgba(0,0,0,0.12)', borderRadius:8, boxShadow:'0 4px 16px rgba(0,0,0,0.1)', zIndex:100, overflow:'hidden', marginBottom:4 }}>
          {suggestions.map(p=>(
            <div key={p.id} onMouseDown={()=>selectMention(p)}
              ...
              <span style={{ fontSize:13, color:'#1a1a1a' }}>{p.full_name}</span>
              <span style={{ fontSize:11, color:'#9b9b9b' }}>@{p.initials}</span>
            </div>
          ))}
        </div>
      )}
          ...
          <div style={{ display:'flex',gap:2,marginBottom:-1,overflowX:'auto' }}>
            {tabs.map(([id,lbl])=>(
              <button key={id} onClick={()=>setActiveTab(id)}
                style={{ padding:'6px 11px',borderRadius:'6px 6px 0 0',border:'none',cursor:'pointer',fontSize:12,fontWeight:500,whiteSpace:'nowrap',transition:'all 150ms',background:activeTab===id?'#fff':'none',color:activeTab===id?'#1a1a1a':'#6b6b6b',borderBottom:activeTab===id?'2px solid #5e6ad2':'2px solid transparent' }}>
                {lbl}
              </button>
            ))}
          </div>
              ...
              : <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                  {taskActivity.map(a=>{const m=actionMeta(a.action); return (
                    <div key={a.id} style={{ display:'flex',gap:10,alignItems:'flex-start' }}>
                      ...
                      <div style={{ flex:1 }}><div style={{ fontSize:13,color:'#1a1a1a' }}>{a.detail}</div><div style={{ fontSize:11,color:'#9b9b9b',marginTop:2 }}>{timeAgo(a.created_at)}</div></div>
                    </div>
                  )})}
                </div>
          )}
    ...
    return text.split('\n').map((line, i) => {
      ...
      return (
        <div key={i} style={{ marginBottom: 2 }}>
          {parts.map((part, j) =>
            j % 2 === 1
              ? <span key={j} style={{ fontWeight: 600, color: '#1a1a1a' }}>{part}</span>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <button
          ...
          style={{
            padding: '7px 16px',
            background: loading ? '#f0efec' : !hasContent ? '#f0efec' : 'linear-gradient(135deg, #5e6ad2, #8b5cf6)',
            color: loading || !hasContent ? '#9b9b9b' : '#fff',
        ...
        >
          {loading ? (
            <>
              <div style={{ width: 12, height: 12, border: '2px solid rgba(0,0,0,0.15)', borderTopColor: '#5e6ad2', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
              Özetleniyor…
            </>
          ) : (
        ...
        <div style={{ background: 'linear-gradient(135deg, #fafafe, #f5f3ff)', border: '1px solid rgba(94,106,210,0.15)', borderRadius: 12, padding: 18 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 10px', background: 'linear-gradient(135deg, #5e6ad2, #8b5cf6)', borderRadius: 99 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#fff', letterSpacing: '0.02em' }}>Claude AI</span>
            </div>
            {lastGenerated && (
              <span style={{ fontSize: 11, color: '#9b9b9b' }}>{lastGenerated.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</span>
            )}
          </div>
          ...
          <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid rgba(94,106,210,0.1)', fontSize: 11, color: '#9b9b9b', display: 'flex', alignItems: 'center', gap: 5 }}>
            <span>⚡</span>
      ...
      {!summary && !loading && hasContent && (
        <div style={{ marginTop: 12, padding: '12px 14px', background: '#f7f7f5', borderRadius: 10, border: '1px solid rgba(0,0,0,0.06)' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#9b9b9b', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Ne analiz edilecek?</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {[
              ...
              <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#6b6b6b' }}>
